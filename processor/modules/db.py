...
DB_PATH = os.path.join(DB_DIR, "mavlink_data.db")
...
def insert_parsed_data(conn, msg_type, timestamp, data):
    cursor = conn.cursor()
    fields = ", ".join(data.keys())
    placeholders = ", ".join(["?" for _ in data.values()])

    # Convert list to JSON string if 'binary' field exists
    if "binary" in data:
        data["binary"] = json.dumps(data["binary"])
...
def insert_message(conn, message):
    ...
    for column in new_columns - existing_columns:
        ...
        cursor.execute(
            f"ALTER TABLE {msg_type} ADD COLUMN {column} {get_sqlite_type(type(data[column]).__name__)}"
        )
    conn.commit()
